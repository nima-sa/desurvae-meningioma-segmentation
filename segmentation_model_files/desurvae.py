...
from segmentation_model_files.unetr_block import UnetrUpBlock
...
def get_conv_layer(spatial_dims, in_channels, out_channels, kernel_size = 3, stride = 1, bias = False):
...
def get_upsample_layer(
    spatial_dims, in_channels, upsample_mode = "nontrainable", scale_factor = 2
):
    return UpSample(
...
class DeSURVAE(nn.Module):
    ...
    def __init__(
        ...
        blocks_up = (1, 1, 1, 1),
        unet_blocks_down=(1, 2, 2, 4),
        include_vae=True,
        include_unet=True,
        unet_dropout_prob=None,
        ...
        self.decoder4 = UnetrUpBlock(
            ...
            norm_name=norm_name,
            res_block=True,
            skip_channels=(include_unet * unet_init_filters * 8 * (add_mirror_convs + 1)),
        )
        ...
        self.decoder3 = UnetrUpBlock(
            ...
            norm_name=norm_name,
            res_block=True,
            skip_channels=(include_unet * unet_init_filters * 4 * (add_mirror_convs + 1)),
        )
        ...
        self.decoder2 = UnetrUpBlock(
            ...
            norm_name=norm_name,
            res_block=True,
            skip_channels=(include_unet * unet_init_filters * 2 * (add_mirror_convs + 1)),
        )
        ...
        self.decoder1 = UnetrUpBlock(
            ...
            norm_name=norm_name,
            res_block=True,
            skip_channels=(include_unet * unet_init_filters * 1 * (add_mirror_convs + 1)),
        )
        ...
        self.smallest_filters = 16 # feature_size

        zoom = 2 ** (len(self.depths) - 1) * 4 # idk what that 4 is. But it is what it is
        ...
        if self.include_vae or add_mirror_convs:
        ...
        if self.include_unet:
            self.init_filters = unet_init_filters
            self.in_channels = in_channels
            self.blocks_down = unet_blocks_down
            self.dropout_prob = unet_dropout_prob

            if add_mirror_convs:
                self.add_mirror_convs = add_mirror_convs
                self.up_layers_mirror, self.up_samples_mirror = self._make_up_layers_mirror()
                self.conv_final_mirror = self._make_final_conv(in_channels)
                # self.convInit_mirror = None
                
            self.down_layers = self._make_down_layers()
            self.convInit = get_conv_layer(spatial_dims, in_channels, self.init_filters)
    ...
    def _prepare_vae_modules(self):
        zoom = 2 ** (len(self.depths) - 1) # 8
        v_filters = self.feature_size * zoom * 2 # 768 idk what that 2 is. But it is what it is
        total_elements = int(self.smallest_filters * np.prod(self.fc_insize)) # 8192, 24576, 1024
    ...
    def _make_up_layers(self):
        ...
        if n_up % 2 == 0:
            ...
            up_samples.append(
                nn.Sequential(
                    *[
                        get_conv_layer(spatial_dims, sample_in_channels // 2, sample_in_channels // 2, kernel_size=1),
                        get_upsample_layer(spatial_dims, sample_in_channels // 2, upsample_mode=upsample_mode),
                    ]
                )
            )
    ...
    def _make_up_layers_mirror(self):
        up_layers, up_samples = nn.ModuleList(), nn.ModuleList()
        upsample_mode, blocks_up, spatial_dims, filters, norm = (
            self.upsample_mode,
            self.blocks_up,
            self.spatial_dims,
            self.init_filters, # different than that in _make_up_layers. IDK WHY IT SHOULD BE !!!!!
            self.norm,
        )
        n_up = len(blocks_up) - 1 # IDK where that -1 comes from. But ... it is what it is...
        self.convInit_mirror = get_conv_layer(spatial_dims, filters * 2 ** n_up, filters * 2 ** n_up)
    ...
    def _get_vae_loss(self, net_input, vae_input):
        """
        Args:
    ...
    def _make_final_conv(self, out_channels):
        return nn.Sequential(
            get_norm_layer(name=self.norm, spatial_dims=self.spatial_dims, channels=self.feature_size),
            self.act,
            get_conv_layer(self.spatial_dims, self.feature_size, out_channels, kernel_size=1, bias=True),
        )
    
    def unet_encode(self, x_in):
        x = x_in
        x = self.convInit(x)
        if self.dropout_prob is not None:
            x = self.dropout(x)
    ...
    def unet_decode(self, x, down_x):
        container = []
        x = self.convInit_mirror(x)
        container.append(x)
        for i, (up, upl) in enumerate(zip(self.up_samples_mirror, self.up_layers_mirror)):
            up_x = up(x)  # Calculate the up-sampled tensor
            down_tensor = down_x[i + 1]
            
            # Check if shapes match
            if up_x.shape != down_tensor.shape:
                # Calculate the padding required for each dimension
                # diff = [max(0, up_x.shape[d] - down_tensor.shape[d]) for d in range(2, len(up_x.shape))]
                # pad = [item for sublist in zip(diff[::-1], diff[::-1]) for item in sublist]
                diff = [up_x.shape[d] - down_tensor.shape[d] for d in range(2, len(up_x.shape))]
                pad = []
                for d in reversed(diff):
                    pad.extend([0, max(0, d)])  # Only pad the end of each dimension
                # Pad the down_tensor to match up_x's shape
                down_tensor = F.pad(down_tensor, pad)
            
            x = up_x + down_tensor  # Merge up-sampled and down-sampled tensors
            x = upl(x)  # Apply the corresponding up-layer
            container.append(x)
    ...
    def forward(self, x_in):
        ...
        enc10 = self.encoder10(hidden_states_out[4])
        
        if self.include_unet:
            down_x = self.unet_encode(x_in)
            if self.add_mirror_convs:
                _, up_x = self.unet_decode(down_x[-1], down_x[::-1])
                for i in range(len(up_x)):
                    down_x[i] = torch.cat((down_x[i], up_x[len(up_x) - 1 - i]), dim=1)

            enc0 = torch.cat((enc0, down_x[0]), dim=1)
            enc1 = torch.cat((enc1, down_x[1]), dim=1)
            enc2 = torch.cat((enc2, down_x[2]), dim=1)
            enc3 = torch.cat((enc3, down_x[3]), dim=1)

        if self.training and self.include_vae:
            vae_loss = self._get_vae_loss(x_in, enc10)
        else:
            vae_loss = None

    
        dec3 = self.decoder5(enc10, hidden_states_out[3])
...
class PatchMergingV2(nn.Module):
    ...
    def __init__(self, dim, norm_layer = nn.LayerNorm, spatial_dims = 3) -> None:
        """
        Args:
            dim: number of feature channels.
            norm_layer: normalization layer.
